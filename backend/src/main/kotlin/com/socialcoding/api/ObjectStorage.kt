...
object ObjectStorage : Initializable {
    ...
    // Loaded from the environment in [initialize]; default to blank so [isConfigured] reads false
    // if anything touches the store before initialization runs.
    ...
    /**
     * The stored file extension for an image [contentType], or null if it isn't
     * a supported type.
     */
    ...
    fun supportsContentType(contentType: String): Boolean =
        extensionFor(contentType) != null

    /**
     * Store [bytes] as an image of [contentType] under a random key and return
     * the URL to display it at — a direct link when `S3_PUBLIC_URL` is set,
     * otherwise the backend proxy path.
     *
     * @throws com.socialcoding.common.ServerError if storage isn't configured, the type is unsupported,
     *   or the store rejects the upload.
     */
    suspend fun uploadImage(bytes: ByteArray, contentType: String): String {
        if (!isConfigured)
            throw ServerError("Image uploads aren't configured on this server.")
        val extension =
            extensionFor(contentType)
                ?: throw ServerError("Unsupported image type: $contentType")
        ...
        return if (publicBase.isNotBlank()) "$publicBase/$key"
        else "/api/images/$key"
    }

    /**
     * PUT an object into the bucket using path-style addressing, signed with
     * AWS Signature V4.
     */
    private suspend fun putObject(
        ...
        // These are exactly the headers the request sends (Host is added by the
        // client from the URL).
        // The ACL header is only present when configured, since not every store
        // accepts it.
        val signed =
            sortedMapOf(
        ...
        val authorization =
            authorization("PUT", key, signed, payloadHash, amzDate, dateStamp)
    ...
    suspend fun getObject(key: String): StoredObject? {
        ...
        val authorization =
            authorization("GET", key, signed, payloadHash, amzDate, dateStamp)
        ...
        if (!response.status.isSuccess()) {
            throw ServerError(
                "Object store returned ${response.status.value} for $key"
            )
        }
        val contentType =
    ...
    /**
     * Build the AWS Signature V4 `Authorization` header for a request. [signed]
     * must contain exactly the headers the request sends, keyed by lowercase
     * name; a sorted map keeps them canonical.
     */
    private fun authorization(
        ...
        val signature =
            hex(hmac(signingKey(dateStamp), stringToSign.toByteArray()))
    ...
    private fun signingKey(dateStamp: String): ByteArray {
        val kDate =
            hmac("AWS4$secretKey".toByteArray(), dateStamp.toByteArray())
    ...
    /**
     * RFC 3986 encoding S3 expects for a path segment (unreserved characters
     * stay literal).
     */
    ...
    override suspend fun initialize() {
        ...
        runCatching { getObject("$KEY_PREFIX.healthcheck-${Uuid.random()}") }
            .onSuccess { log.info("Object storage initialized for bucket {} at {}", bucket, endpoint) }
            .onFailure { log.error("Object storage is configured but unreachable or misconfigured.", it) }
    }
}